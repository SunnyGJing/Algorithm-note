...
1. 是二叉树，并且是二叉搜索树（剑指offer 68）
2. 是二叉树，但不是二叉树（剑指offer 68 - II)  
3. 不是二叉树，有指向父节点的指针  
4. 不是二叉树，没有指向父节点的指针  
...
## 二刷 20年8月
### 解法 for 剑指offer 68
看到**二叉搜索树**，想到**利用结点值进行判断**，根节点的值大于左子树中所有节点的值，根节点的值小于右子树中所有结点的值  
看到**最低公共祖先**，想到**找到某节点，满足结点p和结点q分列该节点左右**
#### 代码
```python
...
class Solution:
    ...
    """
        ...
        return root
```
### 解法 for 剑指offer 68 - II
**本题不能用上面用于搜索树的解法，会超时！**  
转而用**递归**来求解，函数功能转为判断当前根节点是否等于p或q  
则递归结束后返回的是当前子树是否含有p或q  
#### 代码
```python
# Definition for a binary tree node.
# class TreeNode:
