...
class Solution:
    def exist(self, board: List[List[str]], word: str) -> bool:
        ...
        def dfs(i, j, rows, cols, flag, inx):
            if i < 0 or j <0 or i >= rows or j >= cols or \
            board[i][j] != word[inx] or flag[i][j]: return False
            if inx == len(word)-1: return True
            flag[i][j] = True
            cur = dfs(i, j+1, rows, cols, flag, inx+1) or \
                    dfs(i, j-1, rows, cols, flag, inx+1) or \
                    dfs(i-1, j, rows, cols, flag, inx+1) or \
                    dfs(i+1, j, rows, cols, flag, inx+1)
            flag[i][j] = False
            return cur
